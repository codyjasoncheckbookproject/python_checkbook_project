...
def cusaccountcheck():
    ...
    while name not in cusnames and len(name) < 3:
        name = input(
            "Please type in your name for this new bank account\n")
    ...
    while len(pin) < 4:
        pin = input(
            "Please assign a password to this account, pin should be at least 5 characters\n")
        if len(pin) > 4:
            print("your pin has been successfully saved")
            print(
                "Remember to always keep your pin safe and don't disclose it to anybody")
